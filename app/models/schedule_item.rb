class ScheduleItem < ActiveRecord::Base
  ...
  def self.grade_report(student, start_date, end_date)
    self.select("schedule_items.*, marks.date, subjects.name, marks.mark")
      .joins("LEFT JOIN marks ON marks.schedule_item_id = schedule_items.id AND marks.student_id =#{student.id}")
      .includes(:subject)
      .where("schedule_items.student_group_id = ? and marks.date BETWEEN ? and ?", student.student_group_id, start_date, end_date)
      .order(:week_day)
